        ...
        Try
            ...
            For Each lS As Integer In listS
                If String.IsNullOrEmpty(lWhereSocio) = True Then
                    lWhereSocio = String.Format("and d.IdCliente in ({0}", lS)
                Else
            ...
            If PROPeriodoVendedorDetalleBO.GetAllQuery(gConnectionString, String.Format("Select distinct top 1 * From PRO_Periodo as p inner join PRO_PeriodoVendedorDetalle as d on p.IdPeriodo=d.IdPeriodo Where d.IdProducto = {0} {1} and p.PeriodoCerrado = 0", CInt(e.Cell.Row.Cells("IdRecurso").Value), lWhereSocio)).Count > 0 Then
